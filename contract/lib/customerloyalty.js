...
class CustomerLoyalty extends Contract {
  ...
  async CreateMember(ctx, member) {
    ...
    let levelData = await ctx.stub.getState(levelKey);
    ...
    await ctx.stub.putState(levelKey, Buffer.from(JSON.stringify(levelData)));
  ...
  async EarnPoints(ctx, earnPoints) {
    ...
    member.totalPoints += earnPoints.points;
    await ctx.stub.putState(
      earnPoints.member,
      Buffer.from(JSON.stringify(member))
    );
    ...
    let levelData = await ctx.stub.getState(levelKey);
    levelData = JSON.parse(levelData);
    for (let each of levelData) {
      if (each.accountNumber == member.accountNumber) {
        each.totalPoints = member.totalPoints;
        while (member.totalPoints > each.level * 100)
