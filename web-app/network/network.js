...
module.exports = {
  ...
  registerMember: async function(
    ...
    try {
      ...
      const createMemberResponse = await contract.submitTransaction(
        "CreateMember",
  ...
  registerPartner: async function(cardId, partnerId, name) {
    ...
    try {
      ...
      const createPartnerResponse = await contract.submitTransaction(
        "CreatePartner",
      ...
      console.log("\nGet partner state ");
      const partnerResponse = await contract.evaluateTransaction(
        "GetState",
        partnerId
      );
  ...
  earnPointsTransaction: async function(
    ...
    try {
      ...
      const earnPointsResponse = await contract.submitTransaction(
        "EarnPoints",
        JSON.stringify(earnPoints)
      );
  ...
  usePointsTransaction: async function(
    ...
    try {
      ...
      const usePointsResponse = await contract.submitTransaction(
        "UsePoints",
  ...
  allPartnersInfo: async function(cardId) {
    ...
    try {
      ...
      let allPartners = await contract.evaluateTransaction(
        "GetState",
        "all-partners"
      );
   ...
   * @param {String} cardId Card id to connect to network
   */
  levelData: async function(cardId) {
    // Create a new file system based wallet for managing identities.
    const walletPath = path.join(process.cwd(), "/wallet");
    const wallet = new FileSystemWallet(walletPath);
    console.log(`Wallet path: ${walletPath}`);
    ...
    try {
      ...
      console.log("\nGet level data: ");
      let levelData = await contract.evaluateTransaction(
        "GetState",
        "level-key"
      );
      levelData = JSON.parse(levelData.toString());
  ...
  earnPointsTransactionsInfo: async function(cardId, userType, userId) {
    ...
    try {
      ...
      let earnPointsTransactions = await contract.evaluateTransaction(
        "EarnPointsTransactionsInfo",
        userType,
        userId
      );
  ...
  usePointsTransactionsInfo: async function(cardId, userType, userId) {
    ...
    try {
      ...
      let usePointsTransactions = await contract.evaluateTransaction(
        "UsePointsTransactionsInfo",
        userType,
        userId
      );
