...
app.post("/api/registerPartner", function(req, res) {
  ...
  validate
    .validatePartnerRegistration(cardId, partnerId, name)
    .then(response => {
      //return error if error in response
      if (response.error != null) {
        res.json({
        ...
        network.registerPartner(cardId, partnerId, name).then(response => {
          //return error if error in response
          if (response.error != null) {
            res.json({
              error: response.error
...
app.post("/api/earnPoints", function(req, res) {
  ...
  validate.validatePoints(points).then(checkPoints => {
    //return error if error in response
  ...
  validate
    ...
    .then(checkPoints => {
      if (checkPoints.error != null) {
        ...
        network
          .usePointsTransaction(cardId, accountNumber, partnerId, points)
          .then(response => {
            //return error if error in response
            if (response.error != null) {
              res.json({
  ...
  network
    ...
    .then(member => {
      ...
      if (member.error != null) {
        ...
        returnData.totalPoints = member.totalPoints;
      }
    })
    .then(() => {
      //get UsePoints transactions from the network
      network
        .usePointsTransactionsInfo(cardId, "member", accountNumber)
        ...
        .then(() => {
          ...
          network
            .earnPointsTransactionsInfo(cardId, "member", accountNumber)
            ...
            .then(() => {
              ...
              network
                ...
                .then(() => {
                  //get level data
                  network.levelData(cardId).then(levelData => {
                    //return error if error in response
                    if (levelData.error != null) {
                      res.json({
...
app.post("/api/partnerData", function(req, res) {
  ...
  network
    ...
    .then(() => {
      //get UsePoints transactions from the network
      network
        .usePointsTransactionsInfo(cardId, "partner", partnerId)
        .then(usePointsResults => {
          //return error if error in response
          if (usePointsResults.error != null) {
            res.json({
        ...
        .then(() => {
          //get EarnPoints transactions from the network
          network
            .earnPointsTransactionsInfo(cardId, "partner", partnerId)
            .then(earnPointsResults => {
              //return error if error in response
              if (earnPointsResults.error != null) {
                res.json({
